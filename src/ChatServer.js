...
socketio.on("connection", function (socketclient) {
    ...
    socketclient.on("login", async (username,password) => {
        ...
        if(validatePassword(password)&&validateUsername(username)){
            ...
            if (checklogin && userList.includes(username))
            {
                ...
                //CHAT HISTORY IMPLEMENTATION ATTEMPT
    ...
    socketclient.on("boop", function(){
        ...
        randoNum = parseInt((Math.random()*numUsers),10);
        console.log("randoNum is: " + randoNum);
        randomUser = userList[randoNum];
        if(randomUser == socketclient.username){
            if(randoNum <= 1){
                randomUser = userList[randoNum+1];
            }
            else{
                randomUser = userList[randoNum-1];
            }
        ...
        socketclient.emit("boopBuddy",randomUser); 
    ...
    socketclient.on("friend", (friendName) => {
        //XSS PREVENTION
        friendName = xssfilter(friendName);
        ...
        for(let i = 0; i < userList.length; i++ ){
            if(friendName == userList[i]){
                ...
                messengerdb.addFriend(socketclient.username,friendName);
                flag = 1;
            }
            if(flag == 0){
    ...
    socketclient.on("name", (inputName) => {
        //XSS PREVENTION
        inputName = xssfilter(inputName);
        
        if(!socketclient.authenticated) {
            console.log("Unauthenticated client added a name. Suppress!");
            return;
        }
        messengerdb.addName(socketclient.username,inputName);
    });

    socketclient.on("phonenumber", (phoneNumber) => {
        //XSS PREVENTION
        phoneNumber = xssfilter(phoneNumber);

        if(!socketclient.authenticated) {
            console.log("Unauthenticated client added a number. Suppress!");
            return;
        }            
        messengerdb.addNumber(socketclient.username,phoneNumber);
    });

    socketclient.on("email", (email) => {
        //XSS PREVENTION
        ...
        messengerdb.addEmail(socketclient.username,email);
    });
    ...
    socketclient.on("addMember", (groupName, memberName) => {
        //XSS PREVENTION
        groupName = xssfilter(groupName);
        memberName = xssfilter(memberName);
    ...
    socketclient.on("isUserInGroup", async (gChatName) => {
        var tf = false;
        if (!groupList.includes(gChatName)){ //if group does not exist
            ...
            for (var g in socketclient.groups){
                if (socketclient.groups[g].name == gChatName){ //if user in group
                    ...
                    //GROUP HISTORY IMPLEMENTATION ATTEMPT 
                    var group_chat_history = await messengerdb.loadGroupChatHistory(socketclient.username, gChatName)
    ...
    socketclient.on("gchat", (message) => {
            ...
            if (socketclient.groups[groupN].name == socketclient.currentGroup){ //if group == user's currentGroup
                for (var mem in socketclient.groups[groupN].members){ //loop through all the members in the current group
                    if (userList.includes(socketclient.groups[groupN].members[mem])){ //wont msg those in group that disconnected
                        ...
                        memSocket.emit("grpchat", chatmessage); //emit chat to current member's socket
                    }
                    else{
        ...
        messengerdb.storeGroupChat(chatmessage, socketclient.currentGroup);
    });
    ...
    for(var socketId in sockets){
        ...
        if(socketclient.authenticated){
            ...
            if(type=="chat")
                ...
                messengerdb.storePrivateChat(socketclient.username, data);
            else if(type=="gchat")
                messengerdb.storeGroupChat(socketclient.username, data, socketclient.currentGroup);
...
function sendPrivateMessage(senderclient,recipientName,type,data){
    //XSS PREVENTION
    data = xssfilter(data);
    recipientName = xssfilter(recipientName);
    var sockets = socketio.sockets.sockets;
    var tf = false;
    for(var socketId in sockets){
        var socketclient = sockets[socketId];
        if(socketclient.username == recipientName){
            ...
            messengerdb.storePrivateChat(senderclient.username, data);
            messengerdb.storePrivateChat(socketclient.username, data);
